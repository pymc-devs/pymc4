...
def true_joint_lp(x, y):
    return (tf.reduce_sum(tfd.Normal(0,1).log_prob(x)) 
        + tf.reduce_sum(tfd.Normal(x, 1).log_prob(y)))
...
def test_condition_x(simple_model):
    ...
    assert logp_y == pytest.approx(true_joint_lp(x=np.float32([1,1,1]),
                                                y=np.float32([1,2,3])))
...
def y(simple_model):
    ...
    assert logp_x == pytest.approx(true_joint_lp(x=np.float32([1,1,1]),
                                                y=np.float32([1,2,3])))
...
def test_condition_xy(simple_model):
    ...
    logp_func = mod.make_log_prob_function()
    logp_xy = logp_func()
    assert logp_xy == pytest.approx(true_joint_lp(x=np.float32([1,1,1]),
                                                y=np.float32([1,2,3])))