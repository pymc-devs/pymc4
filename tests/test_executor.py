...
def test_transformed_executor_logp_tensorflow():
    ...
    _, state = pm.evaluate_model_transformed(transformed_model(), __log_n=-np.pi)
    np.testing.assert_allclose(state.collect_log_prob(), norm_log.log_prob(-np.pi), equal_nan=False)

    _, state = pm.evaluate_model_transformed(transformed_model(), n=np.exp(-np.pi))
    np.testing.assert_allclose(state.collect_log_prob(), norm_log.log_prob(-np.pi), equal_nan=False)
...
def test_executor_logp_tensorflow():
    ...
    _, state = pm.evaluate_model(transformed_model(), n=np.pi)

    np.testing.assert_allclose(state.collect_log_prob(), norm.log_prob(np.pi), equal_nan=False)
